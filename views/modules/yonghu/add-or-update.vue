		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.touxiang" label="头像" prop="touxiang">
					<img v-if="ruleForm.touxiang.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" :key="'http-' + index" :src="ruleForm.touxiang.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-for="(item, index) in ruleForm.touxiang.split(',')" :key="'local-' + index" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
